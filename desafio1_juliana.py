...
class Submissao:
    ...
    def phi(self, X: torch.Tensor) -> torch.Tensor:
        ...
        if not hasattr(self, "W"):
            ...
            Xs0 = (X - self.mu) / self.sd
            d = X.shape[1]

            n_amostras = min(300, len(Xs0))
            d2 = torch.cdist(Xs0[:n_amostras], Xs0[:n_amostras]) ** 2
            sigma = math.sqrt(d2[d2 > 0].median().item() / 2) if len(d2[d2 > 0]) > 0 else 1.0

            self.n_fina = 6 if d <= 2 else 0
            d_novas = self.DIM_MAX - d - 1 - self.n_fina

            g = torch.Generator().manual_seed(545807482)
            self.W = torch.randn(d, d_novas, generator=g) / sigma
            self.b = torch.rand(d_novas, generator=g) * 2 * math.pi
            if self.n_fina > 0:
                self.W_fina = torch.randn(d, self.n_fina, generator=g) / (sigma / 4)
                self.b_fina = torch.rand(self.n_fina, generator=g) * 2 * math.pi

        Xd = X.double()
        mu, sd = self.mu.double(), self.sd.double()
        Xs = (Xd - mu) / sd
        r2 = (Xs ** 2).sum(dim=1, keepdim=True) / 9.8
        novas_features = 1.2 * torch.sin(Xs @ self.W.double() + self.b.double())
        partes = [Xs, r2, novas_features]
        if self.n_fina > 0:
            partes.append(1.2 * torch.sin(Xs @ self.W_fina.double() + self.b_fina.double()))
        return torch.cat(partes, dim=1).float()
